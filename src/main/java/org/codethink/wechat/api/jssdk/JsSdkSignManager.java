package org.codethink.wechat.api.jssdk;

import javax.servlet.http.HttpServletRequest;

import org.apache.commons.codec.digest.DigestUtils;
import org.codethink.wechat.api.exception.WechatException;
import org.codethink.wechat.api.jssdk.entity.JsSdkSignParam;
...
/**
 * 
 * 调用jssdk需要进行的签名算法相关接口(生成config接口注入权限验证配置使用的signature参数)
 * 
 * @author CaiXiangNing
 * @date 2016年10月17日
...
public class JsSdkSignManager {
	...
	private static JsSdkSignParam signature(String url) throws WechatException {
		...
		// 对参数进行字典序后拼接的字符串作sha1加密，字段名和字段值都采用原始值，不进行URL 转义。
	...
	public static void executeUrlSignature(HttpServletRequest httpServletRequest) throws WechatException {
		...
		// 对请求Url执行签名算法，并将通过config接口注入权限验证配置需要的参数放在httpServletRequest中
